...
app.get('/session/:id', function(req, res) {
		io.sockets.on('connection', function(socket) {
			...
			socket.on('latexChanged', function(data) {
				io.sockets.in(req.params.id).broadcast.emit('latexChanged', {'rawText': data['rawText'], 'sendingUser': data['sendingUser']});
			});
			socket.on('solutionChanged', function(data) {
				io.sockets.in(req.params.id).broadcast.emit('solutionChanged', {'solutionText': data['solutionText'], 'sendingUser': data['sendingUser']});
			});
			socket.on('graphChanged', function(data) {
				io.sockets.in(req.params.id).broadcast.emit('graphChanged', {'graphSource': data['graphSource'], 'sendingUser': data['sendingUser']});
			});
			socket.on('fileUploaded', function(data) {
				io.sockets.in(req.params.id).broadcast.emit('fileUploaded', {'fileSource': data['fileSource'], 'sendingUser': data['sendingUser']});
			});
			socket.on('codeChanged', function(data) {
				io.sockets.in(req.params.id).broadcast.emit('codeChanged', {'code': data['code'], 'sendingUser': data['sendingUser']});
			});
		});
